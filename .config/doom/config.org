...
** Org keybindings
This section defines custom keybindings to improve my Org workflow.

I frequently refile, retag, and update TODO states across multiple Org files
(inbox, projects, tickler). Instead of saving buffers manually one by one,
I bind a dedicated leader key to save all open Org buffers at once.

This keeps my workflow fast and intentional:
- Capture freely
- Triage and refile
- Save all Org buffers with a single command

*** Save all Org buffers
Bind =SPC b o s= to =org-save-all-org-buffers=.

This saves all modified Org buffers without touching non-Org buffers.

#+begin_src emacs-lisp
(after! org
  (map! :leader
        :desc "Save all Org buffers"
        "b o s" #'org-save-all-org-buffers))
#+end_src
...
** Opacity
Set frame background opacity to 90% for a slight transparency effect.
...
Disable the confirmation prompt that fires when a commit summary exceeds 72 characters.
Setting to =nil= causes a format-specifier error, so we use a high number instead.
